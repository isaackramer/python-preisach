...
beta_values = np.around(np.linspace(0, 1, hys_per_side),2) # beta values
alpha_values = np.around(np.linspace(1,0, hys_per_side),2) # alpha values
beta_grid, alpha_grid = np.meshgrid(beta_values, alpha_values)

# create array showing whether hysterons on/off
# inital values: all hysterons set to 0 (negative saturation) and values outside of preisach triangle set to np.nan
preisach_triangle = np.where(alpha_grid>=beta_grid, 0, np.nan)

# assign weights: "uniform", "linear", "bottom_heavy", "top_heavy", "left_heavy", "right_heavy"
# "center_light_alpha", "center_light_beta", "center_heavy_alpha", "center_heavy_beta"
...
plot2 = ax2.pcolormesh(beta_grid, alpha_grid, preisach_triangle,
                       cmap = YlGnBu_9.mpl_colormap)
plot3 = ax3.scatter(beta_grid, alpha_grid, c = mu,
                    cmap = YlGnBu_9.mpl_colormap,
                    s = 0.2)
...
def update(val):
    ...
    outputs, preisach_triangle = preisach_single_value(inputs, alpha_grid, beta_grid, mu,
                                                       preisach_triangle, outputs)
    plot1.set_ydata(outputs)
    plot1.set_xdata(inputs)
    ax2.pcolormesh(beta_grid, alpha_grid, preisach_triangle)
    fig.canvas.draw_idle()

# for the slider to remain responsive you must maintain a reference to it. Call on_changed() to connect to the slider event.
